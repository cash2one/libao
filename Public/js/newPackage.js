		 ...
		 $('#validation-form-step2').validate({
			...
			rules: {
				giftname: {
					...
					remote:{
						type: "POST",             //数据发送方式
		                url: localhostPaht+projectName+"/index.php?m=home&c=Record&a=checkGiftName",     //后台处理程序   
				...
				token: {
					...
					remote:{
						type: "POST",             //数据发送方式
		                url: localhostPaht+projectName+"/index.php?m=home&c=Record&a=checkToken",     //后台处理程序   
		                dataType: "json",           //接受数据格式   
		                data: {                     //要传递的数据
		                    token: function() {
		                        return $("#token").val();
